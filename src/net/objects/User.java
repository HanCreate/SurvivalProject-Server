...
//	private byte playerEventFlags[] = {0, 0, 0, 0, 0, 0, 0, 0};
...
//		friends = new Friend[24];
	...
	public static User loadUser(String username, LobbyServer lobbyServer) throws SQLException {
		...
		if (!users.isEmpty()) {
			...
			for (Card card : cardsList) {
				if (card.getCardIndex() != -1) {
					if (card.getCardPremiumDays() > 0) {
						int secondsLeft = (int) ((card.getCardPremiumExpirationDate().getTime() - System.currentTimeMillis()) / 1000);
						int daysLeft = (int) Math.ceil((float) secondsLeft / 60 / 60 / 24);
						
						if (daysLeft <= 0) {
							daysLeft = 0;
							card.setCardPremiumExpirationDate(null);
						}
						
						card.setCardPremiumDays(daysLeft);
					}
			...
			if (cardsList.isEmpty()) {
				for (int i = 0; i < 96; i++) {
					result.cards[i] = new Card(i, -1, -1, -1, -1);
				}
			}
			
			// Save the cards (premium days change every time the user connects)
		...
		for (int i = 0; i < cardIndexes.length; i++) {
			if (user.cards[cardIndexes[i]] != null) {
				...
				if (!cardsList.isEmpty()) {
					...
					cardsList.get(0).setCardPremiumExpirationDate(user.cards[cardIndexes[i]].getCardPremiumExpirationDate());
					
					session.update(cardsList.get(0));
				}
				else {
					session.save(user.cards[cardIndexes[i]]);
				}
			}
			else {
	...
	public int getEmptyCardSlot() {
		for (int i = 0; i < 96; i++) {
			if (cards[i] == null || cards[i].getCardID() == -1) {
				return i;
			}
		}
...
//	}
	...
	public int getRank() {
		Session session = Database.getSession();
		session.beginTransaction();
		Query<Long> query = session.createQuery("SELECT count(*) FROM User WHERE playerExperience > :experience");
		query.setParameter("experience", playerExperience);
		List<Long> count = (List<Long>) query.list();
		
		session.getTransaction().commit();
		session.close();
		
		return count.get(0).intValue() + 1;
	}
	
	@Transient
	public boolean hasCard(int card) {
		for (int i = 0; i < 96; i++) {
			if (cards[i] != null && cards[i].getCardID() == card) {
				return true;
			}
		}
	...
	@Column(name = "last_visit_bonus")
	public long getLastVisitBonus() {
		return lastVisitBonus;
	}

	public void setLastVisitBonus(long lastVisitBonus) {
		this.lastVisitBonus = lastVisitBonus;
	}
}